...
public class DealsApiControllerTest extends BaseTestClass {
    ...
    void testGetActiveDeals_onValidInput_expectSuccessResponse() {
        // Arrange
        ActiveDealsResponse mockResponse = (ActiveDealsResponse) fileLoader.readFileAsObject("responses/service-success-active-deals-response.json", ActiveDealsResponse.class);
    ...
    @Test
    void testGetPeakDeals_onSuccessfulLookup_expectSuccessResponse() {
        // Arrange
        PeakDealsResponse mockResponse = PeakDealsResponse.builder()
                .peakTimeStart("17:00")
                .peakTimeEnd("21:00")
                .build();

        Mockito.when(dealsService.getPeakDeals())
                .thenReturn(Mono.just(mockResponse)
                .delayElement(Duration.ofMillis(500)));
        ...
        // Assert
                .expectStatus().isEqualTo(200)
                .expectBody(PeakDealsResponse.class)
                .isEqualTo(mockResponse);

        Mockito.verify(dealsService, Mockito.times(1)).getPeakDeals();
    }


    @Test
    void testGetPeakDeals_onNullResponse_expectSuccessResponse() {
        // Arrange
        PeakDealsResponse mockResponse = PeakDealsResponse.builder()
                .peakTimeStart(null)
                .peakTimeEnd(null)
    ...
    @Test
    void testGetPeakDeals_onNullPointerException_expect500InternalServerError() {
        // Arrange
        Mockito.when(dealsService.getPeakDeals())
                .thenReturn(Mono.error(new NullPointerException("Simulated null pointer exception")));
    ...
    @Test
    void testGetPeakDeals_onRuntimeException_expect500InternalServerError() {
        // Arrange
        Mockito.when(dealsService.getPeakDeals())
                .thenReturn(Mono.error(new RuntimeException("Simulated runtime exception")));
